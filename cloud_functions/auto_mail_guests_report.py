...
def fetch_data_from_database(conn, d):
    query = "SELECT visitor_name, organization, relationship, employee_name, in_time, out_time FROM milkmil_guests where in_date='{}';".format(d)
...
def main(request):
    ...
    column_names = ['visitor_name', 'organization', 'relationship', 'employee_name', 'in_time', 'out_time']
    ...
    subject = "Visitors Report - {}".format(today)
    body = "Please find the attached data."
    attachment = MIMEApplication(excel_buffer.getvalue())
    attachment['Content-Disposition'] = 'attachment; filename=Visitors_{}.xlsx'.format(today)
