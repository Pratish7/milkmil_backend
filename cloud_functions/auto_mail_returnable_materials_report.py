...
def fetch_data_from_database(conn, d):
    query = "SELECT out_date, out_time, gate_pass_num, in_date, in_time, status FROM milkmil_returnablematerials where out_date='{}';".format(d)
...
def main(request):
    ...
    column_names = ['Out Date', 'Out Time', 'Gate Pass Number', 'In Date', 'In Time', 'Status']
    ...
    subject = "Returnable Material Report - {}".format(today)
    body = "Please find the attached data."
    attachment = MIMEApplication(excel_buffer.getvalue())
    attachment['Content-Disposition'] = 'attachment; filename=Returnable_Material_{}.xlsx'.format(today)
