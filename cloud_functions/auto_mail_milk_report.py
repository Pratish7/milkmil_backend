...
def fetch_data_from_database(conn, d):
    query = "SELECT date, time, quantity FROM milkmil_milk where date='{}';".format(d)
...
def main(request):
    ...
    column_names = ['Date', 'Time', 'NO OF PACKETS']
    ...
    subject = "Milk Report - {}".format(today)
    body = "Please find the attached data."
    attachment = MIMEApplication(excel_buffer.getvalue())
    attachment['Content-Disposition'] = 'attachment; filename=Milk_{}.xlsx'.format(today)
