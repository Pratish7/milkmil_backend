...
def fetch_data_from_database(conn, d):
    query = "SELECT date, key_type, taken_time, returned_time FROM milkmil_keys where date='{}';".format(d)
...
def main(request):
    ...
    column_names = ['Date', 'Key Type', 'Taken Time', 'Returned Time']
    ...
    subject = "Keys Report - {}".format(today)
    body = "Please find the attached data."
    attachment = MIMEApplication(excel_buffer.getvalue())
    attachment['Content-Disposition'] = 'attachment; filename=Keys_{}.xlsx'.format(today)
