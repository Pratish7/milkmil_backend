...
def fetch_data_from_database(conn, d):
    query = "SELECT date, invoice_num, in_time, out_time FROM milkmil_materialinward where date='{}';".format(d)
...
def main(request):
    ...
    column_names = ['Date', 'Invoice Number', 'In Time', 'Out Time']
    ...
    subject = "Material Inward Report - {}".format(today)
    body = "Please find the attached data."
    attachment = MIMEApplication(excel_buffer.getvalue())
    attachment['Content-Disposition'] = 'attachment; filename=Material_Inward_{}.xlsx'.format(today)
