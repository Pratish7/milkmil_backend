...
def fetch_data_from_database(conn, d):
    query = "SELECT date, time, invoice_num, status  FROM milkmil_materialoutward where date='{}';".format(d)
...
def main(request):
    ...
    column_names = ['Date', 'Time', 'Invoice Number', 'Status']
    ...
    subject = "Material outward Report - {}".format(today)
    body = "Please find the attached data."
    attachment = MIMEApplication(excel_buffer.getvalue())
    attachment['Content-Disposition'] = 'attachment; filename=Material_Outward_{}.xlsx'.format(today)
